/**
 * The map band's decidable parts.
 *
 * The band itself is GL JS and can only run in a browser with a WebGL context,
 * so the three things about it that carry judgement — which stock style each
 * colour scheme gets, which `maki` glyph each category wears, and the viewport
 * that has to hold the current list — live here instead, where they are plain
 * data and a pure function.
 *
 * Nothing in this file imports GL JS, which is also what lets the page keep
 * working when the map doesn't: `components/insights/map-band.tsx` is the only
 * module that touches `mapbox-gl`.
 */
...
/**
 * Stock base styles, uncustomised, one per colour scheme. A tuned style would
 * compete with the scorecard above it; the only brand colour on the map is the
 * orange address pin.
 */
 ...
 * sprite, so the glyph is identical in both base styles and takes its colour
 * from `--foreground` like every other mark on the page.
 *
 * Three categories have no same-named icon and take the nearest one maki has:
 * transit is `rail`, and the two hyphenated names differ only in spelling. The
 * source icon is named above each entry.
 */
export const MAKI_GLYPHS: Record<CategoryId, string> = {
  /** maki `grocery` */
  grocery:
