...
import { recordLookup } from "@/lib/recent-store";
import {
  createSearchSession,
  insightsHref,
  retrieveAddress,
 ...
 * debounce, the keyboard, and which line sits under the input.
 */
...
/**
 * The one line under the input. All three are neutral — none of them is an
 * error the visitor caused, so none of them is `--destructive`.
 */
type Note = "none" | "empty" | "error" | "locating";

const NOTES: Record<Note, string> = {
  none: "",
  empty: "No matches. Try a street address, city, or ZIP.",
  error: "Search is temporarily unavailable.",
  locating: "Locating that address…",
};
...
export function AddressSearch() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        setNote(found.length > 0 ? "none" : "empty");
      } catch {
  ...
  async function pick(suggestion: AddressSuggestion) {
    ...
    try {
      ...
      // the retry below stays inside the session already paid for.
      session.end();
      recordLookup(address);
      router.push(insightsHref(address));
    } catch {
      // The suggestions are kept and the list reopened, so the retry is one
      // click away. Dropping them would leave a visitor holding the right
      // address with no way to ask for it again but to retype it.
      setNote("error");
      setOpen(true);
    }
  }
  ...
  return (
    ...
    >
      ...
      {/* Divs rather than a list, so the listbox owns its options directly: an
          intervening `li` would break the ownership `aria-activedescendant`
          depends on. The options are not tab stops by design — the input keeps
          focus and moves the active descendant, which is how a combobox is
          meant to be driven from the keyboard. */}
      {expanded && (
        <div
          id={listId}
          role="listbox"
        ...
        >
          {suggestions.map((suggestion, index) => (
            <div
              ...
              className={cn(
                "flex cursor-pointer flex-col gap-0.5 border-b px-5 py-3 text-left last:border-b-0",
